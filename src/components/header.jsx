...
export const Header = () => {
    ...
    const logRegAction = () => {
        if(typeWindow===typeLog)
            login({username:data.username, password: data.password}).then(ans=>setNotif(prev=>{return {visible:true, message:'Успешно авторизован'}}))
                                                                    .catch(ans=>setNotif(prev=>{return {visible:true, message:'Не удалось авторизоваться'}}))
        
        if(typeWindow===typeReg)
            reg({username:data.username, password:data.password, email:data.email}).then(ans=>setNotif(prev=>{return {visible:true, message:'Регистрация прошла успешно'}}))
